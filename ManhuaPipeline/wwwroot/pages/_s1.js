...
var currentStage = 0;
var stages = [];
var stageOrder = [1,2,3,4,6,7,8,11,5,9,10];
...
function loadStages(){$.get("/api/project/"+currentProjectId+"/stage/all",function(d){stages=d.stages||[];stageOrder=d.stageOrder&&d.stageOrder.length?d.stageOrder:stageOrder;renderStageMenu();showStage(d.currentStage||1)})}

function getStageName(n){var a=["","创意构思","故事分析","全局蓝图","分集细化","分镜脚本","角色资产","道具资产","环境资产","提示词生成","衔接检查","特效资产"];return a[n]||"未知"}
function getStageIcon(n){var a=["","<i class=\"fas fa-lightbulb\"></i>","<i class=\"fas fa-book-open\"></i>","<i class=\"fas fa-globe\"></i>","<i class=\"fas fa-layer-group\"></i>","<i class=\"fas fa-film\"></i>","<i class=\"fas fa-user\"></i>","<i class=\"fas fa-tools\"></i>","<i class=\"fas fa-mountain\"></i>","<i class=\"fas fa-magic\"></i>","<i class=\"fas fa-link\"></i>","<i class=\"fas fa-wand-magic-sparkles\"></i>"];return a[n]||"<i class=\"fas fa-question\"></i>"}function renderStageMenu(){
  var h="";
  ...
  for(var k=0;k<stageOrder.length;k++){var i=stageOrder[k];var s=stages[i-1]||{};var st=s.status||"pending";var ic=getStageIcon(i);var nm=getStageName(i);var stCls=st==="completed"?" done":st==="processing"?" processing":st==="failed"?" failed":"";var cls="menu-item"+(i===currentStage?" active":"")+stCls
...
function showStage(num){
  ...
  else if(num===6||num===7||num===8||num===11){$("#episodeSection").hide();$("#frameSection").hide();$("#assetSection").hide();$("#stageResult").removeClass("no-content").html("<p style='color:#94a3b8;padding:20px;'>加载中...</p>").show();loadAssets(num)}
